...
class MoveEmptyToTile(Move):
    ...
    def end(self, tile):
        ...
        if self.delay > TILE_GEN_DELAY:
            ...
            tl = Tile(pygame.Color(TILE_TYPE[color_id]), tile.rect.width, tile.board)
        ...
        for y in range(board_height):
            for x in range(board_width):
                if playmap[y][x] == '*':
                    ...
                    tile = TileEmpty(EMPTY_COLOR, tile_size, self)
                    tile.rect.x = margin_side + tile.rect.width * x
                    tile.rect.y = SCREEN_MARGIN_TOP + tile.rect.height * y
                    add_text('empty', tile)
                    self.empty.add(tile)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
            ...
            for i in self.tiles:
                if i.rect.collidepoint(event.pos):
                    ...
                    #ts=get_neighbour(i,DIR_DICT['up'],self.tiles)
                    #if ts:
                    #    ts.board.tiles.remove(ts)
